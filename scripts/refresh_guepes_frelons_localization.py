...
def build_local_chunk(city_slug: str, service_key: str):
    ...
    if service_key == "guepes":
        neigh_tpl = [
            f"<h2>Infestation de {s['label']} dans le quartier {q1} à {city}</h2><p>Dans {q1}, les appels concernent surtout des nids actifs près des avancées de toit, des terrasses et des coffres de volets. Les zones mixtes (immeubles, pavillons, commerces) demandent une gestion fine des flux piétons. La sécurisation du passage est prioritaire avant toute action, notamment près des écoles et des entrées d'immeuble.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q2} à {city}</h2><p>Le secteur {q2} présente des habitats variés où les guêpes exploitent les haies, annexes et abris légers. Les interventions sont ajustées selon la hauteur, l'accessibilité et l'usage des lieux (restauration, commerces, cours résidentielles). Cette adaptation évite les reprises rapides pendant les périodes chaudes.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q3} à {city}</h2><p>Dans {q3}, les contraintes d'accès (ruelles, stationnement serré, cours fermées) imposent un balisage rigoureux. Le protocole combine repérage visuel, traitement ciblé et contrôle des points périphériques. L'objectif est de sécuriser durablement l'environnement immédiat des occupants et des riverains.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q4} à {city}</h2><p>Le quartier {q4} cumule des jardins, dépendances et façades exposées où l'activité peut passer inaperçue au départ. Un diagnostic local complet permet de traiter le foyer principal puis de vérifier les zones satellites. Cette méthode réduit le risque de retour au cours de la même saison.</p>",
        ]
        ...
        neigh_tpl = [
            f"<h2>Infestation de {s['label']} dans le quartier {q1} à {city}</h2><p>Dans {q1}, les interventions concernent régulièrement des nids positionnés en hauteur sur charpentes, arbres, sous-toitures et annexes de jardin. L'évaluation du danger est renforcée près des axes de circulation, des établissements scolaires et des entrées de copropriété.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q2} à {city}</h2><p>Le secteur {q2} combine bâtiments collectifs, maisons et zones commerciales. Les frelons s'installent dans des volumes peu perturbés et exigent une intervention encadrée avec contrôle des accès techniques. La procédure varie selon la hauteur et la possibilité d'approche sécurisée.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q3} à {city}</h2><p>Dans {q3}, les contraintes d'accès peuvent être fortes: cours fermées, toits difficiles, passages étroits. L'équipe prépare un balisage strict avant traitement et vérifie les points périphériques où des individus restent actifs. Cette rigueur réduit les risques pendant et après l'opération.</p>",
            f"<h2>Infestation de {s['label']} dans le quartier {q4} à {city}</h2><p>Le quartier {q4} présente des zones végétalisées et des dépendances qui favorisent des implantations discrètes. Le diagnostic complet permet d'identifier le nid principal, d'évaluer l'exposition des occupants et de mettre en place un passage complémentaire si nécessaire.</p>",
        ]
...
{''.join(neigh_tpl)}
<h2>Pourquoi les {s['label']} s'installent à {city}</h2>
<p>{why}</p>
<h2>Risques liés aux {s['label']} à {city}</h2>
<p>{risk}</p>
<h2>Traitement professionnel contre les {s['label']} à {city}</h2>
<p>{treatment}</p>

<section class="card" data-rich-local>
  <h2>{s['action'].capitalize()} à {city} : analyse locale avancée</h2><p>{card1}</p><p>Cette méthode améliore la lisibilité pour le client et renforce l\'efficacité opérationnelle, notamment dans les zones où l\'accès est contraint.</p>
</section>

<section class="card" data-rich-local>
  <h2>Quartiers sensibles à {city} : priorisation opérationnelle</h2><p>{card2}</p><p>Ce découpage par micro-secteur limite les retours rapides et évite les interventions dispersées sans logique de suivi.</p>
</section>
...
<section class="card" data-rich-local>
  <h2>Plan d'action recommandé pour {s['label']} à {city}</h2>
  <ol>
...
def update_schema_faq(text: str, city: str, service_key: str):
    ...
    qa = []
    for q, a in s['faq']:
        qa.append('{"@type":"Question","name":"'+q+' à '+city+' ?","acceptedAnswer":{"@type":"Answer","text":"'+a+'"}}')
    new_main = '"mainEntity": [\n          ' + ',\n          '.join(qa) + '\n        ]'
